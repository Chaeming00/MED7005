...
def get_main_parser():
    ...
    # Dataset parameters #####################################################################################################################################################################
    parser.add_argument('--dataset',                    type=str,           default='stnet_dataset',            help='Dataset to use.', choices=['stnet_dataset', 'V1_Breast_Cancer_Block_A', 'V1_Human_Brain', 'HEST_dataset'])
    ...
    ##########################################################################################################################################################################################
    # Train and gene file arguments
    parser.add_argument('--train_path', type=str, default=None, help='Path to the training CSV file.')
    parser.add_argument(
        '--gene_files',
        type=str,
        help="Path to gene files in JSON string format. Example: '{\"var\": \"/path/to/var_50genes.json\"}'"
    )


    return parser

from datasets import HESTDataset  # HESTDataset 클래스 가져오기

def get_dataset_from_args(args) -> object:
    """
    ...
    Returns:
        dataset: The specified dataset object.
    """
    # gene_files의 타입 확인 및 변환
    if hasattr(args, "gene_files") and isinstance(args.gene_files, str):
        import json
        try:
            args.gene_files = json.loads(args.gene_files)
            print("Gene Files (after parsing):", args.gene_files)
        except json.JSONDecodeError:
            raise ValueError("Invalid JSON format for --gene_files. Ensure it is a valid JSON string.")

    # HEST 데이터셋 사용 여부 확인
    if args.dataset == "HEST_dataset":
        if not hasattr(args, "gene_files") or "var" not in args.gene_files:
            raise ValueError("Missing or invalid 'gene_files' argument. Ensure 'var' key is provided.")
        
        dataset = HESTDataset(
            csv_path=args.train_path,
            base_dir="/home/guest3/projects/SEPAL/HEST/bench_data/CCRCC",
            gene_file=args.gene_files["var"]
        )
    else:
        # 기존 SpatialDataset 생성
        param_dict = {
...
def train_simple(model, loader, criterion, optimizer, transforms):
    """
    Train function for a simple model with a dataloader.
    ...
    Args:
        ...
        optimizer: Optimizer.
        transforms: Transformations to apply to input images.
    """
    # Ensure model is in training mode
    model.train()

    # Loop through the training data
    for batch_idx, (tissue_tiles, expression_gt) in enumerate(tqdm(loader, desc="Training")):
        # Move data to the same device as the model
        device = next(model.parameters()).device
        tissue_tiles = tissue_tiles.to(device)
        expression_gt = expression_gt.to(device)

        # Normalize image tiles
        tissue_tiles = tissue_tiles / 255.0
        ...
        print(f"Expression GT shape: {expression_gt.shape}")

        # Compute loss
        loss = criterion(expression_pred, expression_gt)
        ...
        # Print loss (optional, for debugging purposes)
...
def update_save_metric_df(metric_df, epoch, train_metric_dict, path):
    """
    Update and save the metrics dataframe for training.

    Args:
        metric_df (pd.DataFrame): Current metrics dataframe.
        epoch (int): Current epoch.
        train_metric_dict (dict): Dictionary with training metrics.
        path (str): Path to save the metrics dataframe.

    Returns:
        pd.DataFrame: Updated metrics dataframe.
    """
    # Copy train metric dict
    train_dict = train_metric_dict.copy()

    # Remove keys that can not be stored in dataframe
    train_dict.pop('pearson_series', None)

    # Add train prefix to keys
    ...
    curr_df = pd.DataFrame(train_dict)

    # If this is the first step then initialize metric_df and save with header
    if metric_df is None:
        metric_df = curr_df
        metric_df.to_csv(path, index=False)
    else:
        # Append the new row to the existing dataframe and save without header
        metric_df = pd.concat([metric_df, curr_df], ignore_index=True)
        curr_df.to_csv(path, mode='a', index=False, header=False)

    # Define a refined print string for terminal output
    print_str = f'Epoch {epoch} '
    for key in sorted(train_dict):
        if key == 'epoch':
            continue
        print_str += f'| {key} = {round(train_dict[key][0], 4)}'
