...
class ImageEncoder(torch.nn.Module):
    def __init__(self, backbone, use_pretrained,  latent_dim):
        ...
        # Fully Connected Layer to map latent_dim to (max_cells * num_genes)
        self.fc = nn.Linear(self.latent_dim, self.latent_dim)

    def forward(self, tissue_tiles):
        latent_space = self.encoder(tissue_tiles)  # (batch_size, latent_dim)
        output = self.fc(latent_space)  # (batch_size, max_cells * num_genes)
        return output
...
class STNet(torch.nn.Module):
    ...
    def forward(self, tissue_tiles):
        # ImageEncoder를 사용하여 출력 생성
        out = self.encoder(tissue_tiles)  # (batch_size, max_cells, num_genes)
        return out
