...
class HESTPretrainDataset(Dataset):
    def __init__(self, data: pd.DataFrame, gene_file: str):
        """
        Custom Dataset for pretraining HEST backbone.
        Args:
            data (pd.DataFrame): DataFrame containing 'patches_path' and 'expr_path'.
            gene_file (str): Path to JSON file containing the list of genes to filter.
        """
        self.data = data

        # gene_file이 파일 경로인지 확인
        if not os.path.isfile(gene_file):
            raise FileNotFoundError(f"gene_file is not a valid file path: {gene_file}")

        # JSON 파일에서 유전자 목록 로드
        try:
            with open(gene_file, 'r') as f:
                genes_data = json.load(f)
                self.genes = genes_data.get("genes", [])
        except Exception as e:
            raise RuntimeError(f"Error reading gene_file: {e}")

        if not self.genes:
            raise ValueError("No genes found in the provided gene file.")
    ...
    def __getitem__(self, idx):
        """
        Loads the patch and expression data for a given index.
        Args:
            idx (int): Index of the sample to load.
        Returns:
            patch (Tensor): Image patch tensor.
            expr (Tensor): Filtered gene expression tensor.
        """
        sample = self.data.iloc[idx]

        # Load patch and expression data
        patch = self._load_patch(sample['patches_path'])
        expr = self._load_expr(sample['expr_path'])

        return patch, expr

    def _load_patch(self, patch_path):
        """
        Loads the image patch data from an h5 file.
        Args:
            patch_path (str): Path to the patch h5 file.
        Returns:
            Tensor: Normalized patch data.
        """
        try:
            with h5py.File(patch_path, 'r') as f:
                patch = np.array(f['img'])  # 'img' 키에서 데이터 로드

            # 데이터의 차원을 확인하고 적절히 처리
            if patch.ndim == 4:  # (N, H, W, C) 형식일 경우
                patch = patch[0]  # 첫 번째 이미지만 사용하거나, N=1로 가정
            elif patch.ndim != 3:  # 다른 차원의 데이터가 들어오면 예외 처리
                raise ValueError(f"Unexpected patch dimensions: {patch.shape}")

            # Normalize patch (example: scaling to 0-1 range)
            patch = patch.astype(np.float32) / 255.0

            # Convert to Tensor and permute dimensions if necessary
            return torch.tensor(patch).permute(2, 0, 1)  # HWC -> CHW
        except Exception as e:
            raise RuntimeError(f"Error loading patch from {patch_path}: {e}")

    def _load_expr(self, expr_path):
        """
        Loads and filters gene expression data from an h5ad file.
        Args:
            expr_path (str): Path to the expression h5ad file.
        Returns:
            Tensor: Filtered gene expression data.
        """
        try:
            ...
            if missing_genes:
                print(f"Warning: The following genes are missing and will be ignored: {missing_genes}")
            valid_genes = [gene for gene in self.genes if gene in adata.var_names]
            ...
            # Filter gene matrix using indices
            gene_indices = [adata.var_names.tolist().index(gene) for gene in valid_genes]
            filtered_expr = expr_matrix[:, gene_indices]

            # **추가된 디버깅 코드**
            print(f"Sample {expr_path} filtered shape: {filtered_expr.shape}")
            print(f"Number of valid genes: {len(valid_genes)}")

            return torch.tensor(filtered_expr, dtype=torch.float32)
        except Exception as e:
            raise RuntimeError(f"Error loading expression data from {expr_path}: {e}")


class HESTDataset:
    def __init__(self, csv_path: str, base_dir: str, gene_file: str):
        """
        Initialize HEST dataset loader with gene filtering.
        Args:
            csv_path (str): "/home/guest3/projects/SEPAL/HEST/bench_data/CCRCC/splits/train_0.csv"
            base_dir (str): "/home/guest3/projects/SEPAL/HEST/bench_data/CCRCC"
            gene_file (str): "/home/guest3/projects/SEPAL/HEST/bench_data/CCRCC/var_50genes.json"
        """
        # 경로 유효성 확인
        ...
        if 'patches_path' not in self.data.columns or 'expr_path' not in self.data.columns:
            raise ValueError(f"CSV file must contain 'patches_path' and 'expr_path' columns.")

        self.data['patches_path'] = self.data['patches_path'].apply(
            lambda x: os.path.join(base_dir, x)  # 중복된 'patches' 추가를 방지
        )
        self.data['expr_path'] = self.data['expr_path'].apply(
            lambda x: os.path.join(base_dir, x)  # 중복된 'adata' 추가를 방지
        )

        # 사전 처리된 리스트 생성
        self.patches_paths = self.get_patches_path()
        self.expr_paths = self.get_expr_path()

        # 유전자 데이터 로드
        try:
            with open(gene_file, 'r') as f:
                self.genes = json.load(f).get("genes", [])
        except json.JSONDecodeError:
            raise ValueError(f"Gene file is not a valid JSON: {gene_file}")
        
        if not self.genes:
            raise ValueError("Gene file does not contain any genes.")
        
        # AnnData 객체 로드 (첫 번째 expr_path 사용)
        expr_path = self.data['expr_path'].iloc[0]
        try:
            self.adata = anndata.read_h5ad(expr_path)
        except Exception as e:
            raise RuntimeError(f"Error loading AnnData from {expr_path}: {e}")
    ...
    def __getitem__(self, idx):
        ...
        Returns:
            tuple: (patch_tensor, gene_expression_tensor)
        """
        if torch.is_tensor(idx):
            idx = idx.tolist()

         # 사전 처리된 리스트에서 경로 가져오기
        patch_path = self.data['patches_path'].iloc[idx]
        expr_path = self.data['expr_path'].iloc[idx]

        # HDF5 패치 파일 로드
        if patch_path.endswith('.h5'):
            with h5py.File(patch_path, 'r') as f:
                img_array = f['img'][:]  # 모든 이미지 가져오기
                img_tensor = torch.tensor(img_array[0], dtype=torch.float32)  # 첫 번째 이미지 예시
                img_tensor = img_tensor.permute(2, 0, 1)  # [H, W, C] → [C, H, W]
                patch_tensor = img_tensor / 255.0  # 정규화
        elif patch_path.endswith('.pt'):
            # PyTorch 텐서 파일
            patch_tensor = torch.load(patch_path)
        else:
            raise ValueError("Unsupported patch file format. Must be '.h5' or '.pt'")

        # Load gene expression and filter genes
        adata = anndata.read_h5ad(expr_path)

        # Ensure adata.X is transformed to a full array
        if hasattr(adata.X, 'toarray'):
            gene_matrix = adata.X.toarray()
        else:
            gene_matrix = adata.X

        # Print original matrix shape for debugging
        print(f"Original expression matrix shape: {gene_matrix.shape}")
        print(f"Original var_names length: {len(adata.var_names)}")

        common_genes = [gene for gene in adata.var_names.tolist() if gene in self.genes]
        if not common_genes:
            raise ValueError("No matching genes found between gene file and gene matrix.")
        
        gene_indices = [adata.var_names.tolist().index(gene) for gene in common_genes]
        filtered_gene_matrix = gene_matrix[:, gene_indices]

        gene_expression_tensor = torch.tensor(filtered_gene_matrix, dtype=torch.float32)

        print(f"Filtered gene matrix shape: {gene_expression_tensor.shape}")
        
        return patch_tensor, gene_expression_tensor

    def get_patches_path(self) -> list:
        """Returns list of patches paths."""
        return self.data['patches_path'].tolist()

    def get_expr_path(self) -> list:
        """Returns list of expression paths."""
        return self.data['expr_path'].tolist()

    def filter_genes(self, gene_matrix: pd.DataFrame) -> pd.DataFrame:
        """
        Filters the input gene expression matrix to keep only the selected genes.
        Args:
            gene_matrix (pd.DataFrame): Gene expression matrix (rows: samples, columns: genes).
        Returns:
            pd.DataFrame: Filtered gene expression matrix.
        """
        if not isinstance(gene_matrix, pd.DataFrame):
            raise TypeError("Input gene_matrix must be a pandas DataFrame.")
        if gene_matrix.empty:
            raise ValueError("Input gene_matrix is empty.")
        
        # 유전자 리스트로 필터링
        filtered_matrix = gene_matrix.loc[:, gene_matrix.columns.isin(self.genes)]
        if filtered_matrix.empty:
            raise ValueError("No matching genes found in the gene matrix.")
        return filtered_matrix
    
    def get_pretrain_dataloaders(self, layer='d_deltas', batch_size=128, shuffle=True, use_cuda=False):
        # 'split' 열 확인
        if 'split' not in self.data.columns:
            print("Warning: 'split' column not found. Setting all rows to 'train'.")
            self.data['split'] = 'train'  # 기본값으로 설정

        # train split만 사용한다고 가정
        train_data = self.data[self.data['split'] == 'train']

        # JSON 파일 경로를 직접 전달
        gene_file = '/home/guest3/projects/SEPAL/HEST/bench_data/CCRCC/var_50genes.json'
        train_dataset = HESTPretrainDataset(train_data, gene_file=gene_file)

        # validation, test dataset은 사용하지 않으므로 None
        val_dataset = None
        test_dataset = None
        ...
        if 'val' in self.data['split'].unique():
            val_data = self.data[self.data['split'] == 'val']
            val_loader = DataLoader(val_data, batch_size=batch_size, shuffle=False)
        else:
            val_loader = None  # Validation 데이터가 없을 경우

        # Test DataLoader 생성
        if 'test' in self.data['split'].unique():
            test_data = self.data[self.data['split'] == 'test']
            test_loader = DataLoader(test_data, batch_size=batch_size, shuffle=False)
        else:
            test_loader = None  # Test 데이터가 없을 경우
...
def dynamic_padding_collate_fn(batch):
    """
    Custom collate function for dynamic padding of tensors with different sizes.
    
    Args:
        batch (list): List of (patch, expr) tuples
    
    Returns:
        tuple: Padded patches and expressions
    """
    # Separate patches and expressions
    patches, expressions = zip(*batch)
    
    # Find max dimensions
    max_patch_height = max(patch.size(1) for patch in patches)
    max_patch_width = max(patch.size(2) for patch in patches)
    max_expr_len = max(expr.size(0) for expr in expressions)
    
    # Pad patches
    padded_patches = []
    for patch in patches:
        # Pad height and width
        pad_height = max_patch_height - patch.size(1)
        pad_width = max_patch_width - patch.size(2)
        padded_patch = F.pad(patch, (0, pad_width, 0, pad_height))
        padded_patches.append(padded_patch)
    
    # Pad expressions
    padded_expressions = []
    for expr in expressions:
        # Pad to max length of first dimension
        pad_size = max_expr_len - expr.size(0)
        if pad_size > 0:
            padded_expr = torch.cat([expr, torch.zeros(pad_size, expr.size(1), device=expr.device)])
        else:
            padded_expr = expr
        padded_expressions.append(padded_expr)
    
    # Stack padded tensors
    batched_patches = torch.stack(padded_patches)
    batched_expressions = torch.stack(padded_expressions)
    
    return batched_patches, batched_expressions
