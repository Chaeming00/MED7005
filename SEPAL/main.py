...
# 확인용 출력
print(f"Pretrained Image Encoder Path: {args.pretrained_ie_path}")
...
# Get dataloaders
train_loader, test_loader = dataset.get_graph_dataloaders(
...
def main():
    ...
    for epoch in pbar:
        ...
        pbar.set_description(f'Epoch: {epoch} |  Loss Train: {train_loss:.4f}')

    # Load best model and test it
    model.load_state_dict(best_model_wts)
    if test_loader is not None:
        test_metric_dict = test_model(epoch=args.epochs, model=model, test_loader=test_loader)
